...
enum MessageType
{
  ...
  AODVTYPE_F_RREQ  = 5   //!< The fake RREQ packet with a fake destination address
...
Fake Route Request (F_RREQ) Message Format
  ...
  |                           F_RREQ ID                            |
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  |               Fake Destination IP Address                     |
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  |             Fake Destination Sequence Number                  |
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  |                    Originator IP Address                      |
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  |                  Originator Sequence Number                   |
...
class FRreqHeader : public Header
{
public:
  /// c-tor
  FRreqHeader (uint8_t flags = 0, uint8_t reserved = 0, uint8_t hopCount = 0,
              uint32_t requestID = 0, Ipv4Address dst = Ipv4Address (),
              uint32_t dstSeqNo = 0, Ipv4Address origin = Ipv4Address (),
              uint32_t originSeqNo = 0);
  ...
  // Flags
  void SetIsFRequest (bool f);
  bool GetIsFRequest () const;
  void SetGratiousRrep (bool f);
  bool GetGratiousRrep () const;
  void SetDestinationOnly (bool f);
  bool GetDestinationOnly () const;
  ...
  bool operator== (FRreqHeader const & o) const;
private:
  uint8_t        m_flags;          ///< |J|R|G|D|U| bit flags, see RFC
  uint8_t        m_reserved;       ///< Not used
  uint8_t        m_hopCount;       ///< Hop Count
  uint32_t       m_requestID;      ///< F_RREQ ID
  Ipv4Address    m_dst;            ///< Fake Destination IP Address
  uint32_t       m_dstSeqNo;       ///< Fake Destination Sequence Number
  Ipv4Address    m_origin;         ///< Originator IP Address
  uint32_t       m_originSeqNo;    ///< Source Sequence Number
};

std::ostream & operator<< (std::ostream & os, FRreqHeader const &);
...
private:
  uint8_t       m_flags;                  ///< A - acknowledgment required flag and F (for fake request flag)
...
public:
  ...
  // Black Hole flag
  void SetBlackHole (bool f);
  bool GetBlackHole () const;

  /**
   * Add a Black Hole node address and its sequence number in RERR header
   */
  bool AddBlackHole (Ipv4Address dst, uint32_t seqNo);

  /** Delete pair (address + sequence number) from REER header, if the number of Blackhole node > 0
   * \return true on success
   */
  bool RemoveBlackHole (std::pair<Ipv4Address, uint32_t> & un);
...
private:
  ...
  uint8_t m_reserved;        ///< Not used
  uint8_t b_flag;           /// Black Hole Flag
