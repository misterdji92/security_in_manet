...
RoutingProtocol::RoutingProtocol () :
  ...
  FRequestReply(false),    /// For the FRREQ
  EnableHello (false),
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    .AddAttribute ("GratuitousReply", "Indicates whether a gratuitous RREP should be unicast to the node originated route discovery.",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("FRequestReply", "Indicates that the RREP packet coming is from a FRREQ packet.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&RoutingProtocol::SetFRequestReplyFlag,
                                        &RoutingProtocol::GetFRequestReplyFlag),
                   MakeBooleanChecker ())
...
/**** Added By Djibril Sambel DIALLO (University of Ghana) ****/




void
RoutingProtocol::SendFRequest (RrepHeader const & rrepHeader, RoutingTableEntry const & toOrigin)
{
  NS_LOG_FUNCTION (this << toOrigin.GetDestination ());
  std :: cout <<"Inside a send Fake Request . . . \n";
  /*
   * Destination node MUST increment its own sequence number by one if the sequence number in the RREQ packet is equal to that
   * incremented value. Otherwise, the destination does not change its sequence number before generating the  RREP message.
   */
   RoutingTableEntry rt;
    Ipv4Address dst = rrepHeader.GetOrigin();
       m_rreqCount++;
     // Create Fake RREQ header
     FRreqHeader freqHeader;
     freqHeader.SetDst (dst);

   /*  RoutingTableEntry rt;
   //  if (m_routingTable.LookupRoute (dst, rt))  /// We dont need to check if we have an entry for that route bcs it is a fake destination
     //  { */
         m_routingTable.LookupRoute (dst, rt);
        freqHeader.SetHopCount (0);     // freqHeader.SetHopCount (rt.GetHop ());
         if (rt.GetValidSeqNo ())    /// We dont even need to generate a sequence number or look for it we can just make it unknown
           freqHeader.SetDstSeqno (rt.GetSeqNo ()-1);
         else
           freqHeader.SetUnknownSeqno (true);


     m_seqNo++;
     freqHeader.SetOriginSeqno (m_seqNo);
     m_requestId++;
     freqHeader.SetId (m_requestId);
     freqHeader.SetHopCount (0);


  Ptr<Packet> packet = Create<Packet> ();
  packet->AddHeader (freqHeader);
  TypeHeader tHeader (AODVTYPE_F_RREQ);
  packet->AddHeader (tHeader);
...
/**** Added By Djibril Sambel DIALLO (University of Ghana) ****/
   case AODVTYPE_F_RREQ:
        {
          RecvFRequest (packet, receiver, sender);
          break;
        }
          ...
          if (IsMalicious || (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID))
            {
              ...
              */
              if(IsMalicious)
              {
                ...
                SendReplyByIntermediateNode (falseToDst, toOrigin, rreqHeader.GetGratiousRrep (), fakereq);
                 return;
              }
              /* Code for Blackhole Attack Simulation ends here */
              SendReplyByIntermediateNode (toDst, toOrigin, rreqHeader.GetGratiousRrep (), fakereq);
              return;
            }
...
void
RoutingProtocol::RecvFRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  NS_LOG_FUNCTION (this);
  std :: cout <<"inside RecvFRequest . . . \n";

  FRreqHeader freqHeader;
  p->RemoveHeader (freqHeader);
std :: cout <<"the value of GetIsFRequest"<<freqHeader.GetIsFRequest()<<" . . . \n";
  // A node ignores all Fake RREQs received from any node in its blacklist
/*  RoutingTableEntry toPrev;
  if (m_routingTable.LookupRoute (src, toPrev)) ///if we have been able to get a reply through that route it is mean
    {                                          /// that the route is bidirectional so we dont need this method
      if (toPrev.IsUnidirectional ())
        {
          NS_LOG_DEBUG ("Ignoring Fake RREQ from node in blacklist");
          return;
        }
    } */

  uint32_t id = freqHeader.GetId ();    ///Was used in the IsDuplicate method
  Ipv4Address origin = freqHeader.GetOrigin ();

  /*
   *  Node checks to determine whether it has received a RREQ with the same Originator IP Address and RREQ ID.
   *  If such a RREQ has been received, the node silently discards the newly received RREQ.
   */
  if (m_rreqIdCache.IsDuplicate (origin, id))  ///The fake RREQ is never broadcasted so there is no duplicate Packet
                                                 /// in that route therefore we dont need this method
    {
      NS_LOG_DEBUG ("Ignoring Fake RREQ due to duplicate");
      return;
    }

  // Increment Fake RREQ hop count
  uint8_t hop = freqHeader.GetHopCount () + 1;
  freqHeader.SetHopCount (hop);
  freqHeader.SetIsFRequest (true);
  //std :: cout <<"the value of GetIsFRequest is "<<freqHeader.GetIsFRequest()<<" . . . \n";
  ...
  if (!m_routingTable.LookupRoute (origin, toOrigin))
    {
      Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
      RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ origin, /*validSeno=*/ true, /*seqNo=*/ freqHeader.GetOriginSeqno (),
                                              /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0), /*hops=*/ hop,
                                              /*nextHop*/ src, /*timeLife=*/ Time ((2 * NetTraversalTime - 2 * hop * NodeTraversalTime)));
      m_routingTable.AddRoute (newEntry);
    }
  else
    {
      if (toOrigin.GetValidSeqNo ())
        {
          if (int32_t (freqHeader.GetOriginSeqno ()) - int32_t (toOrigin.GetSeqNo ()) > 0)
            toOrigin.SetSeqNo (freqHeader.GetOriginSeqno ());
        }
      else
        toOrigin.SetSeqNo (freqHeader.GetOriginSeqno ());
      toOrigin.SetValidSeqNo (true);
  ...
  if (!m_routingTable.LookupRoute (src, toNeighbor))
    {
      NS_LOG_DEBUG ("Neighbor:" << src << " not found in routing table. Creating an entry");
      Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
      RoutingTableEntry newEntry (dev, src, false, freqHeader.GetOriginSeqno (),
                                              m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                                              1, src, ActiveRouteTimeout);
      m_routingTable.AddRoute (newEntry);
    }
  else
    {
      toNeighbor.SetLifeTime (ActiveRouteTimeout);
      toNeighbor.SetValidSeqNo (false);
      toNeighbor.SetSeqNo (freqHeader.GetOriginSeqno ());
      toNeighbor.SetFlag (VALID);
      toNeighbor.SetOutputDevice (m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver)));
      toNeighbor.SetInterface (m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0));
      toNeighbor.SetHop (1);
  ...
  NS_LOG_LOGIC (receiver << " receive F RREQ with hop count " << static_cast<uint32_t>(freqHeader.GetHopCount ())
                         << " ID " << freqHeader.GetId ()
                         << " to destination " << freqHeader.GetDst ());

  //  A node generates a RREP if either:
  //  (i)  it is itself the destination,
/*  if (IsMalicious || IsMyOwnAddress (freqHeader.GetDst ()))
    {
      std :: cout <<"inside recvfreq . . . \n";
      m_routingTable.LookupRoute (origin, toOrigin);
      NS_LOG_DEBUG ("Send reply since I am the destination");
      SendFReqReply (freqHeader, toOrigin);
      return;
    } */
   ...
   */
  RoutingTableEntry toDst;
  Ipv4Address dst = freqHeader.GetDst ();

  /**** Added By Djibril Sambel DIALLO (University of Ghana) ****/

  if (IsMalicious || m_routingTable.LookupRoute (dst, toDst))
    { std :: cout <<"inside Ismalicious RecvcFReq. . . \n";
      /*
       * Drop F_RREQ, This node RREP will make a loop.
       */
      if (toDst.GetNextHop () == src)
        {
          NS_LOG_DEBUG ("Drop F_RREQ from " << src << ", dest next hop " << toDst.GetNextHop ());
          return;
        }
      /*
       * The Destination Sequence number for the requested destination is set to the maximum of the corresponding value
       * received in the RREQ message, and the destination sequence value currently maintained by the node for the requested destination.
       * However, the forwarding node MUST NOT modify its maintained value for the destination sequence number, even if the value
       * received in the incoming fake RREQ is larger than the value currently maintained by the forwarding node.
       */
  /*    if (IsMalicious || ((freqHeader.GetUnknownSeqno () || (int32_t (toDst.GetSeqNo ()) - int32_t (freqHeader.GetDstSeqno ()) >= 0))
          && toDst.GetValidSeqNo ()) )
        { */
      /*    if (IsMalicious || (!freqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID))
            {
              m_routingTable.LookupRoute (origin, toOrigin); */
              /* Code added by Shalini Satre, Wireless Information Networking Group (WiNG), NITK Surathkal for simulating Blackhole Attack
               * If node is malicious, it creates false routing table entry having sequence number much higher than
               * that in F_RREQ message and hop count as 1.
               * Malicious node itself sends the RREP message,
               * so that the route will be established through malicious node.
              */
              if(IsMalicious)
              { std :: cout <<"malicious sending reply . . . \n";
                Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
                RoutingTableEntry falseToDst(dev,dst,true,freqHeader.GetDstSeqno()+ 100, m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver),0),1,dst,ActiveRouteTimeout);


              std :: cout <<"the value of GetIsFRequest in recvreq is "<<freqHeader.GetIsFRequest()<<" . . . \n";
            //  SendReplyByIntermediateNode (falseToDst, toOrigin, freqHeader.GetGratiousRrep (), freqHeader.GetIsFRequest ());
              // RoutingTableEntry toOrigin;
                m_routingTable.LookupRoute (origin, toOrigin);
              ...
              /* Code for Blackhole Attack Simulation ends here */
              SendReplyByIntermediateNode (toDst, toOrigin, freqHeader.GetGratiousRrep (), freqHeader.GetIsFRequest ());
              return;
            }
          freqHeader.SetDstSeqno (toDst.GetSeqNo ());
          freqHeader.SetUnknownSeqno (false);
       }
  //  }
...
void
RoutingProtocol::SendFReqReply (FRreqHeader const & freqHeader, RoutingTableEntry const & toOrigin)
{
  NS_LOG_FUNCTION (this << toOrigin.GetDestination ());
  std :: cout <<"Inside sendFRequestreply . . . \n";

  /*
   * Destination node MUST increment its own sequence number by one if the sequence number in the RREQ packet is equal to that
   * incremented value. Otherwise, the destination does not change its sequence number before generating the  RREP message.
   */
  if (!freqHeader.GetUnknownSeqno () && (freqHeader.GetDstSeqno () == m_seqNo + 1))
    m_seqNo++;
  RrepHeader rrepHeader ( /*prefixSize=*/ 0, /*hops=*/ 0, /*dst=*/ freqHeader.GetOrigin (),
                                          /*dstSeqNo=*/ m_seqNo, /*origin=*/ toOrigin.GetDestination (), /*lifeTime=*/ MyRouteTimeout);
  std :: cout <<"A node is about to generate a reply for a FRequest . . . \n";
   rrepHeader.SetIsFRequest(true);
...
void
RoutingProtocol::SendReplyByIntermediateNode (RoutingTableEntry & toDst, RoutingTableEntry & toOrigin, bool gratRep, bool fakeReq)
{
  NS_LOG_FUNCTION (this);
   ...
   */
//  std :: cout <<"the value of GetIsFRequest inside sendrepybyinter is "<<fakeReq<<" . . . \n";
   if(fakeReq){
     rrepHeader.SetIsFRequest(true);
//  std :: cout <<"the value of GetIsFRequest of rrepHeader inside sendrepybyinter is "<<rrepHeader.GetIsFRequest()<<" . . . \n";
   }
//std :: cout <<"the value of GetIsFRequest of rrepHeader inside sendrepybyinter is "<<rrepHeader.GetIsFRequest()<<" . . . \n";
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
 ...
 std :: cout <<"Inside RecvReply . . . \n";
  RrepHeader rrepHeader;
  p->RemoveHeader (rrepHeader);
  Ipv4Address dst = rrepHeader.GetDst ();
  NS_LOG_LOGIC ("RREP destination " << dst << " RREP origin " << rrepHeader.GetOrigin ());
// std :: cout <<"the value of GetIsFRequest of rrepHeader is "<<rrepHeader.GetIsFRequest()<<" . . . \n";
...
/**** Added By Djibril Sambel DIALLO (University of Ghana) ****/
  ...
  if (IsMyOwnAddress (rrepHeader.GetDst()))
    {
      std :: cout <<"Inside IsMyOwnAddress . . . \n";

      /* In case where the destination sequence number in the RREP packet is far greater than the
         destination sequence number in the late RREQ packet then we send a fake RREQ packet
         in order to check the authenticity of the node
      */
      std :: cout <<"the value of GetIsFRequest of rrepHeader is "<<rrepHeader.GetIsFRequest()<<" . . . \n";
       if (rrepHeader.GetIsFRequest ()){
         std :: cout <<"Inside GetIsFRequest  of rrepHeader. . . \n";
         //rrepHeader.SetIsFRequest(false);
         SendRerrWhenBlackHoleAttack (p, receiver, sender);

        return;


       }
       else if(rrepHeader.GetDstSeqno () >= toDst.GetSeqNo () + 90)    //If the reply is from a Fake Request packet
       {
         std :: cout <<"Inside comparison of rrepSeq and rreqSeq . . . \n";
         std :: cout <<"RREP destination " << dst << " RREP origin " << rrepHeader.GetOrigin ()<<" . . . \n";
        RoutingTableEntry toOrigin;
          m_routingTable.LookupRoute (dst, toOrigin);
           SendFRequest(rrepHeader, toOrigin);    /*We dont need toDst.GetSeqNo () and toDst.GetRoute ()*/
...
void
RoutingProtocol::SendRerrWhenBlackHoleAttack (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  NS_LOG_FUNCTION (this << " src " << sender);
  std :: cout <<"Inside the SendRerrWhenBlackHoleAttack . . . \n";
  RrepHeader rrepHeader;
  p->RemoveHeader (rrepHeader);
  Ipv4Address dst = rrepHeader.GetDst ();
  NS_LOG_LOGIC ("RREP destination " << dst << " RREP origin " << rrepHeader.GetOrigin ());
...
/*  RerrHeader rerrHeader;
  rerrHeader.AddUnDestination (rrepHeader.GetOrigin (), rrepHeader.GetSeqNo());
  RoutingTableEntry toOrigin;
  Ptr<Packet> packet = Create<Packet> ();
...
/*  RerrHeader rerrHeader;
  ...
  m_routingTable.GetListOfDestinationWithNextHop (rrepHeader.GetOrigin (), dstWithNextHopSrc);
  std::pair<Ipv4Address, uint32_t> un;
  while (rerrHeader.RemoveBlackHole (un))
    {
  ...
  /*  std::vector<Ipv4Address> precursors;
    for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
         i != unreachable.end ();)
      { */
      /*  if (!rerrHeader.AddBlackHole (i->first, i->second))
          { */
          // rrepHeader.GetDstSeqno ()
            RerrHeader rerrHeader;
            rerrHeader.AddBlackHole (rrepHeader.GetOrigin (), rrepHeader.GetDstSeqno());
            //rerrHeader.SetBlackHole (true);
            std::map<Ipv4Address, uint32_t> unreachable ;
          //  unreachable.first = rrepHeader.GetOrigin ();
          //  unreachable.second = rrepHeader.GetDstSeqno();
          //  unreachable = std::make_pair(rrepHeader.GetOrigin (), rrepHeader.GetDstSeqno());
           unreachable.insert(std::make_pair (rrepHeader.GetOrigin (), rrepHeader.GetDstSeqno()));
            /*std::pair<Ipv4Address, uint32_t> un ;
            ...
            RoutingTableEntry toDst;
            std::vector<Ipv4Address> precursors;
            m_routingTable.LookupRoute (rrepHeader.GetOrigin (), toDst);
...
{ std :: cout <<"Inside SendRerrWhenBreaksLinkToNextHop . . . \n";
...
{ std :: cout <<"Inside the SendRerrWhenNoRouteToForward . . . \n";
  NS_LOG_FUNCTION (this);
...
{
  NS_LOG_FUNCTION (this);
 std :: cout <<"Inside SendRerrMessage . . . \n";
  if (precursors.empty ())
    {
