...
// Fake RREQ
//-----------------------------------------------------------------------------
FRreqHeader::FRreqHeader (uint8_t flags, uint8_t reserved, uint8_t hopCount, uint32_t requestID, Ipv4Address dst,
                        uint32_t dstSeqNo, Ipv4Address origin, uint32_t originSeqNo) :
  m_flags (flags), m_reserved (reserved), m_hopCount (hopCount), m_requestID (requestID), m_dst (dst),
  m_dstSeqNo (dstSeqNo), m_origin (origin),  m_originSeqNo (originSeqNo)
{
...
NS_OBJECT_ENSURE_REGISTERED (FRreqHeader);

TypeId
FRreqHeader::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::aodv::FRreqHeader")
    .SetParent<Header> ()
    .SetGroupName("Aodv")
    .AddConstructor<FRreqHeader> ()
  ;
  return tid;
}

TypeId
FRreqHeader::GetInstanceTypeId () const
{
  return GetTypeId ();
}

uint32_t
FRreqHeader::GetSerializedSize () const
{
  return 23;
}

void
FRreqHeader::Serialize (Buffer::Iterator i) const
{
  i.WriteU8 (m_flags);
  i.WriteU8 (m_reserved);
  i.WriteU8 (m_hopCount);
...
uint32_t
FRreqHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
  m_flags = i.ReadU8 ();
  m_reserved = i.ReadU8 ();
...
void
FRreqHeader::Print (std::ostream &os) const
{
  os << "Fake RREQ ID " << m_requestID << "fake destination: ipv4 " << m_dst
     << "fake sequence number " << m_dstSeqNo << " source: ipv4 "
     << m_origin << " sequence number " << m_originSeqNo
     << " flags:" << " RREP from a fake request" << (*this).GetIsFRequest ()
...
std::ostream &
operator<< (std::ostream & os, FRreqHeader const & h)
{
  h.Print (os);
  return os;
}
...
void
FRreqHeader::SetIsFRequest (bool f )
{
  if (f)
    m_flags |= (1 << 6);
  else
...
bool
FRreqHeader::GetIsFRequest () const
{
  return (m_flags & (1 << 6));
}
...
void
FRreqHeader::SetGratiousRrep (bool f)
{
  if (f)
    m_flags |= (1 << 5);
  else
...
bool
FRreqHeader::GetGratiousRrep () const
{
  return (m_flags & (1 << 5));
}

void
FRreqHeader::SetDestinationOnly (bool f)
{
  if (f)
    m_flags |= (1 << 4);
  else
...
bool
FRreqHeader::GetDestinationOnly () const
{
  return (m_flags & (1 << 4));
}

void
FRreqHeader::SetUnknownSeqno (bool f)
{
  if (f)
    m_flags |= (1 << 3);
  else
...
bool
FRreqHeader::GetUnknownSeqno () const
{
  return (m_flags & (1 << 3));
}

bool
FRreqHeader::operator== (FRreqHeader const & o) const
{
  return (m_flags == o.m_flags && m_reserved == o.m_reserved &&
          m_hopCount == o.m_hopCount && m_requestID == o.m_requestID &&
          m_dst == o.m_dst && m_dstSeqNo == o.m_dstSeqNo &&
...
void
RrepHeader::SetIsFRequest (bool f )
{
  if (f)
    m_flags |= (1 << 7);
  else
...
bool
RrepHeader::GetIsFRequest () const
{
  return (m_flags & (1 << 7));
}
...
RerrHeader::RerrHeader () :
  m_flag (0), m_reserved (0), b_flag (0)
{
}
...
RerrHeader::Print (std::ostream &os ) const
{
  ...
  os << "Black  Hole flag" << (*this).GetBlackHole();
}
...
void
RerrHeader::SetBlackHole (bool f )
{
  if (f)
    b_flag |= (1 << 0);
  else
...
bool
RerrHeader::AddBlackHole (Ipv4Address dst, uint32_t seqNo)
{
  SetBlackHole (true);
  return AddUnDestination (dst, seqNo);
}

bool
RerrHeader::RemoveBlackHole (std::pair<Ipv4Address, uint32_t> & un )
{
...
{
  if (b_flag != o.b_flag || m_flag != o.m_flag || m_reserved != o.m_reserved || GetDestCount () != o.GetDestCount ())
    return false;
